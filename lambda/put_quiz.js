...
exports.handler = async function(event, context) {
  ...
  if (!quiz || !quiz.questions || !quiz.questions.length || !quiz.topTitle ||
      !quiz.bottomTitle || !quiz.leftTitle || !quiz.rightTitle) {
    console.log(quiz);
    return getDTO(400, "invalid format");
  }
  ...
  for (let q of quiz.questions) {
    ...
    for (let a of q.options) {
      if (!a.text || a.xValue === null || a.xValue === undefined ||
          a.yValue === null || a.yValue === undefined || !a.id) {
        console.log(q.options);
        return getDTO(400, "invalid answer format");
      }
    }
  }
  ...
  for (const slur of slurs) {
    if (lbody.includes(slur)) {
      console.log(`${slur} @ ${lbody.indexOf(slur)}`);
      return getDTO(400, "lets keep it clean");
    }
  }
  // generate quiz id
  quiz.id = uuidv4();
  ...
  try {
    ...
    return getDTO(200, "success", quiz);
  } catch (e) {
